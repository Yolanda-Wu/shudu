...
router.get("/api/ranks", async (ctx, next) => {
  ...
  data = fs.readFileSync("ranks.json", "utf-8");
...
router.get("/:filename", async (ctx, next) => {
  ...
  ctx.response.body = fs.readFileSync("../src" + url, "utf-8");
});

router.post("/api/ranks", async (ctx, next) => {
  const username = ctx.request.body.username || "";
  let time = Number(ctx.request.body.time) || "";
  let data = fs.readFileSync("./ranks.json");
  ...
  fs.writeFileSync("./ranks.json", JSON.stringify(newData));
